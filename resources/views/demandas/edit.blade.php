...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="bi bi-pencil"></i> Editar Demanda</h2>
                        ...
                        <select class="form-select @error('cliente_id') is-invalid @enderror" id="cliente_id"
                            ...
                            @foreach ($clientes as $cliente)
                                <option value="{{ $cliente->id }}"
                                    {{ old('cliente_id', $demanda->cliente_id) == $cliente->id ? 'selected' : '' }}>
                                    {{ $cliente->nome }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                        ...
                        <select class="form-select @error('solicitante_id') is-invalid @enderror" id="solicitante_id"
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ old('solicitante_id', $demanda->solicitante_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('responsavel_id') is-invalid @enderror" id="responsavel_id"
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ old('responsavel_id', $demanda->responsavel_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->nome }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-select @error('status_id') is-invalid @enderror" id="status_id" name="status_id"
                            ...
                            @foreach ($statuses as $status)
                                <option value="{{ $status->id }}"
                                    {{ old('status_id', $demanda->status_id) == $status->id ? 'selected' : '' }}>
                                    {{ $status->nome }}
                                </option>
                            @endforeach
                        </select>
